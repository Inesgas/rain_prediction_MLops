...
RESULTS_DIR = PROJECT_ROOT / "models" / "gru_sequence_benchmark"
RESULTS_PATH = RESULTS_DIR / "gru_candidate_results.csv"
SUMMARY_PATH = RESULTS_DIR / "gru_summary.json"
HISTORY_PATH = RESULTS_DIR / "gru_training_history.csv"
THRESHOLD_CURVE_PATH = RESULTS_DIR / "gru_threshold_curve.csv"
...
LEARNING_CURVE_FIGURE = FIGURES_DIR / "fig_54_gru_learning_curve.png"
THRESHOLD_FIGURE = FIGURES_DIR / "fig_55_gru_threshold_curve.png"
COMPARISON_FIGURE = FIGURES_DIR / "fig_56_gru_vs_winner_comparison.png"
...
CANDIDATE_CONFIGS: list[dict[str, Any]] = [
    {
        "candidate": "gru_context_7_regularized",
        "model_label": "Temporal GRU",
        "lookback": 7,
        "hidden_size": 64,
        "num_layers": 1,
        "dropout": 0.10,
    ...
    {
        "candidate": "gru_context_14_regularized",
        "model_label": "Temporal GRU",
        "lookback": 14,
        "hidden_size": 64,
        "num_layers": 1,
    ...
    {
        "candidate": "gru_context_21_balanced",
        "model_label": "Temporal GRU",
        "lookback": 21,
        "hidden_size": 64,
        "num_layers": 1,
...
def candidate_display_label(candidate: str) -> str:
    labels = {
        "gru_context_7_regularized": "Temporal GRU, 7-day regularized",
        "gru_context_14_regularized": "Temporal GRU, 14-day regularized",
        "gru_context_14_deep": "Temporal GRU, 14-day deeper stack",
        "gru_context_21_balanced": "Temporal GRU, 21-day balanced",
        "gru_context_21_deep": "Temporal GRU, 21-day deeper stack",
    }
...
def write_notes() -> None:
    ...
    text = """# Temporal GRU Benchmark

## Goal

Test whether a GRU-based temporal sequence model can outperform the final CatBoost winner when both models are built
...
The benchmark uses a CPU-only PyTorch GRU because the project environment does not include a GPU runtime. The
...
def make_model(config: dict[str, Any], input_size: int) -> GRUSequenceClassifier:
...
def save_learning_curve(history: pd.DataFrame) -> None:
    ...
    axes[0].set_title("GRU training stability")
    axes[0].grid(alpha=0.25)
    axes[0].legend(frameon=False)

    axes[1].plot(history["epoch"], history["validation_roc_auc"], color="#8e5ea2", linewidth=2)
    axes[1].set_xlabel("Epoch")
    axes[1].set_ylabel("Validation ROC-AUC")
    axes[1].set_title("GRU validation discrimination by epoch")
...
def save_threshold_curve(frame: pd.DataFrame) -> None:
    ...
    ax.set_ylabel("Score")
    ax.set_title("GRU validation threshold sweep")
    ax.grid(alpha=0.25)
...
def save_comparison_figure(gru_metrics: dict[str, float], winner_metrics: dict[str, float]) -> None:
    FIGURES_DIR.mkdir(parents=True, exist_ok=True)
    ...
    gru_values = [float(gru_metrics[key]) for key in metric_keys]
    winner_values = [float(winner_metrics[key]) for key in metric_keys]
    positions = np.arange(len(metric_keys))
    width = 0.35

    fig, ax = plt.subplots(figsize=(10.2, 4.8))
    ax.bar(positions - width / 2, gru_values, width=width, label="Temporal GRU", color="#7d3c98")
    ...
    ax.set_ylim(0.0, 1.0)
    ax.set_ylabel("Score")
    ax.set_title("Temporal GRU versus final CatBoost winner")
...
def run_experiment() -> dict[str, Any]:
    ...
    summary = {
        "benchmark_name": "gru_sequence_benchmark",
        ...
        "winner_comparison": {
            ...
            "gru_minus_winner_roc_auc": float(best_result["winner_test_roc_auc_gap"]),
            "gru_minus_winner_f1": float(best_result["winner_test_f1_gap"]),
            "gru_minus_winner_precision": float(best_result["winner_test_precision_gap"]),
            "gru_minus_winner_recall": float(best_result["winner_test_recall_gap"]),
        },
        "figure_paths": {
