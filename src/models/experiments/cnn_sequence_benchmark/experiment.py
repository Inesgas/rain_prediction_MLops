...
RESULTS_DIR = PROJECT_ROOT / "models" / "cnn_sequence_benchmark"
RESULTS_PATH = RESULTS_DIR / "cnn_candidate_results.csv"
SUMMARY_PATH = RESULTS_DIR / "cnn_summary.json"
HISTORY_PATH = RESULTS_DIR / "cnn_training_history.csv"
THRESHOLD_CURVE_PATH = RESULTS_DIR / "cnn_threshold_curve.csv"
...
LEARNING_CURVE_FIGURE = FIGURES_DIR / "fig_60_cnn_learning_curve.png"
THRESHOLD_FIGURE = FIGURES_DIR / "fig_61_cnn_threshold_curve.png"
COMPARISON_FIGURE = FIGURES_DIR / "fig_62_cnn_vs_winner_comparison.png"
...
CANDIDATE_CONFIGS: list[dict[str, Any]] = [
    {
        "candidate": "cnn_context_14_light",
        "model_label": "Temporal CNN",
        "lookback": 14,
        "projection_dim": 48,
        "channels": (48, 64),
        "kernel_size": 3,
    ...
    {
        "candidate": "cnn_context_14_dilated",
        "model_label": "Temporal CNN",
        "lookback": 14,
        "projection_dim": 64,
        "channels": (64, 64, 96),
...
def candidate_display_label(candidate: str) -> str:
    labels = {
        "cnn_context_14_light": "Temporal CNN, 14-day light",
        "cnn_context_14_dilated": "Temporal CNN, 14-day dilated",
    }
    return labels.get(candidate, candidate.replace("_", " ").title())


def selected_candidate_configs() -> list[dict[str, Any]]:
    requested = os.environ.get("CNN_CANDIDATES", "").strip()
    if not requested:
        return list(CANDIDATE_CONFIGS)
    requested_names = {item.strip() for item in requested.split(",") if item.strip()}
    selected = [config for config in CANDIDATE_CONFIGS if str(config["candidate"]) in requested_names]
    if not selected:
        raise ValueError(f"No CNN candidates matched CNN_CANDIDATES={requested!r}")
    return selected


def write_notes() -> None:
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    text = """# Temporal CNN Benchmark

## Goal

Test whether a temporal convolutional model can outperform the final CatBoost winner when both models are built
...
The benchmark uses a CPU-only PyTorch temporal CNN because the project environment does not include a GPU runtime.
The benchmark is therefore a real temporal deep-learning model, but still sized for reproducibility in the current
repository while keeping the exact same validation-first chronological selection rule used elsewhere in the project.
"""
...
class TemporalCNNClassifier(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.input_projection = nn.Linear(input_size, int(projection_dim))
        layers: list[nn.Module] = []
        in_channels = int(projection_dim)
        ...
        pooled_size = int(channels[-1]) * 2
        self.head = nn.Sequential(
            nn.Linear(pooled_size, int(channels[-1])),
            nn.GELU(),
            nn.Dropout(float(dropout)),
            nn.Linear(int(channels[-1]), 1),
        )
...
def make_model(config: dict[str, Any], input_size: int) -> TemporalCNNClassifier:
...
def save_learning_curve(history: pd.DataFrame) -> None:
    ...
    axes[0].set_title("CNN training stability")
    axes[0].grid(alpha=0.25)
    axes[0].legend(frameon=False)

    axes[1].plot(history["epoch"], history["validation_roc_auc"], color="#a23b2a", linewidth=2)
    axes[1].set_xlabel("Epoch")
    axes[1].set_ylabel("Validation ROC-AUC")
    axes[1].set_title("CNN validation discrimination by epoch")
...
def save_comparison_figure(cnn_metrics: dict[str, float], winner_metrics: dict[str, float]) -> None:
    FIGURES_DIR.mkdir(parents=True, exist_ok=True)
    ...
    cnn_values = [float(cnn_metrics[key]) for key in metric_keys]
    winner_values = [float(winner_metrics[key]) for key in metric_keys]
    positions = np.arange(len(metric_keys))
    width = 0.35

    fig, ax = plt.subplots(figsize=(10.2, 4.8))
    ax.bar(positions - width / 2, cnn_values, width=width, label="Temporal CNN", color="#c65d00")
    ...
    ax.set_ylim(0.0, 1.0)
    ax.set_ylabel("Score")
    ax.set_title("Temporal CNN versus final CatBoost winner")
...
def run_experiment() -> dict[str, Any]:
    ...
    active_configs = selected_candidate_configs()
    ...
    for config in active_configs:
        print(f"[cnn] starting {config['candidate']}", flush=True)
        ...
        print(
            f"[cnn] finished {config['candidate']} validation_f1={fit_result['validation_metrics']['f1']:.4f} "
        ...
        results_rows.append(
            {
                ...
                "lookback": lookback,
                "projection_dim": int(config["projection_dim"]),
                "channels": "-".join(str(int(value)) for value in config["channels"]),
                "kernel_size": int(config["kernel_size"]),
                "dilation_base": int(config["dilation_base"]),
                "dropout": float(config["dropout"]),
        ...
        pd.concat(histories, axis=0, ignore_index=True).to_csv(HISTORY_PATH, index=False)
        threshold_results.to_csv(RESULTS_DIR / f"{config['candidate']}_threshold_curve.csv", index=False)
    ...
    summary = {
        "benchmark_name": "cnn_sequence_benchmark",
        "feature_set_name": BEST_FEATURE_SET_NAME,
        "evaluated_candidates": [str(config["candidate"]) for config in active_configs],
        "feature_count": len(features),
        "initial_transformed_feature_count": initial_transformed_feature_count,
        "final_transformed_feature_count": final_transformed_feature_count,
        ...
        "winner_comparison": {
            ...
            "cnn_minus_winner_roc_auc": float(best_result["winner_test_roc_auc_gap"]),
            "cnn_minus_winner_f1": float(best_result["winner_test_f1_gap"]),
            "cnn_minus_winner_precision": float(best_result["winner_test_precision_gap"]),
            "cnn_minus_winner_recall": float(best_result["winner_test_recall_gap"]),
        },
        "figure_paths": {
