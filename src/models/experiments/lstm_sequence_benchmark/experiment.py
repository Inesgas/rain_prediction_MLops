...
RESULTS_DIR = PROJECT_ROOT / "models" / "lstm_sequence_benchmark"
RESULTS_PATH = RESULTS_DIR / "lstm_candidate_results.csv"
SUMMARY_PATH = RESULTS_DIR / "lstm_summary.json"
HISTORY_PATH = RESULTS_DIR / "lstm_training_history.csv"
THRESHOLD_CURVE_PATH = RESULTS_DIR / "lstm_threshold_curve.csv"
...
LEARNING_CURVE_FIGURE = FIGURES_DIR / "fig_47_lstm_learning_curve.png"
THRESHOLD_FIGURE = FIGURES_DIR / "fig_48_lstm_threshold_curve.png"
COMPARISON_FIGURE = FIGURES_DIR / "fig_49_lstm_vs_winner_comparison.png"
...
CANDIDATE_CONFIGS: list[dict[str, Any]] = [
    {
        "candidate": "lstm_context_7",
        "model_label": "Temporal LSTM",
        "lookback": 7,
        "hidden_size": 48,
        "num_layers": 1,
        "dropout": 0.0,
    ...
    {
        "candidate": "lstm_context_7_regularized",
        "model_label": "Temporal LSTM",
        "lookback": 7,
        "hidden_size": 64,
        "num_layers": 1,
    ...
    {
        "candidate": "lstm_context_14_regularized",
        "model_label": "Temporal LSTM",
        "lookback": 14,
        "hidden_size": 64,
        "num_layers": 1,
    ...
    {
        "candidate": "lstm_context_14_deep",
        "model_label": "Temporal LSTM",
        "lookback": 14,
        "hidden_size": 80,
        "num_layers": 2,
    ...
    {
        "candidate": "lstm_context_21_balanced",
        "model_label": "Temporal LSTM",
        "lookback": 21,
        "hidden_size": 64,
        "num_layers": 1,
...
def candidate_display_label(candidate: str) -> str:
    labels = {
        "lstm_context_7": "Temporal LSTM, 7-day context",
        "lstm_context_7_regularized": "Temporal LSTM, 7-day regularized",
        "lstm_context_14_regularized": "Temporal LSTM, 14-day regularized",
        "lstm_context_14_deep": "Temporal LSTM, 14-day deeper stack",
        "lstm_context_21_balanced": "Temporal LSTM, 21-day balanced",
    }
...
def write_notes() -> None:
    ...
    text = """# Temporal LSTM Benchmark

## Goal

Test whether an explicit temporal-sequence model can outperform the final CatBoost winner when both models are built
from the same hybrid-plus-core winner feature space.
...
The benchmark uses a CPU-only PyTorch LSTM because the project environment does not include a GPU runtime. The
benchmark is therefore a real temporal deep-learning model, but still sized for reproducibility in the current
repository. The optimized run expands the recurrent search space and training schedule while keeping the exact same
validation-first chronological selection rule used by the rest of the project.
"""
...
def build_row_preprocessor(X: pd.DataFrame) -> ColumnTransformer:
...
def prepare_partition_frames() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, list[str]]:
    ...
    keep_cols = list(dict.fromkeys(["location", "date", TARGET] + features))
    return (
        train_df[keep_cols].copy(),
        valid_df[keep_cols].copy(),
...
def transform_partitions(
    ...
    if fit_on == "train_valid":
        fit_frame = pd.concat([train_df, valid_df], axis=0, ignore_index=True)
    else:
        fit_frame = train_df

    preprocessor = build_row_preprocessor(fit_frame[features])
    preprocessor.fit(fit_frame[features])

    combined = pd.concat([train_df, valid_df, test_df], axis=0, ignore_index=True).copy()
    combined["split"] = (
        ["train"] * len(train_df)
        + ["valid"] * len(valid_df)
        + ["test"] * len(test_df)
    ...
    return preprocessor, vectors, combined
...
def build_sequence_splits(
    ...
    frame = frame.sort_values(["location", "date", "source_index"]).reset_index(drop=True)
    ...
    for _, group in frame.groupby("location", sort=False):
        ...
        targets = group[TARGET].astype(int).to_numpy()
        ...
        for position in range(lookback - 1, len(group)):
            ...
            target_rows.setdefault(split_name, []).append(int(targets[position]))
    ...
    for split_name, sequences in sequence_rows.items():
        ...
        y = np.asarray(target_rows[split_name], dtype=np.float32)
        result[split_name] = (X, y)
        result["counts"][split_name] = int(len(y))
    return result
...
def make_model(config: dict[str, Any], input_size: int) -> LSTMSequenceClassifier:
...
def predict_probabilities(model: nn.Module, X: np.ndarray, batch_size: int) -> np.ndarray:
...
def threshold_frame(proba: np.ndarray, y_true: np.ndarray) -> pd.DataFrame:
    rows: list[dict[str, float]] = []
    y_series = pd.Series(y_true.astype(int))
    for threshold in np.arange(0.30, 0.71, 0.02):
        scores = score_predictions(y_series, proba, threshold=float(threshold))
        rows.append(
            {
...
def accuracy_from_proba(y_true: np.ndarray, proba: np.ndarray, threshold: float = 0.5) -> float:
...
def save_learning_curve(history: pd.DataFrame) -> None:
    ...
    axes[0].set_title("LSTM training stability")
    axes[0].grid(alpha=0.25)
    axes[0].legend(frameon=False)

    axes[1].plot(history["epoch"], history["validation_roc_auc"], color="#0d7a5f", linewidth=2)
    axes[1].set_xlabel("Epoch")
    axes[1].set_ylabel("Validation ROC-AUC")
    axes[1].set_title("LSTM validation discrimination by epoch")
...
def save_threshold_curve(frame: pd.DataFrame) -> None:
    ...
    ax.set_ylabel("Score")
    ax.set_title("LSTM validation threshold sweep")
    ax.grid(alpha=0.25)
...
def save_comparison_figure(lstm_metrics: dict[str, float], winner_metrics: dict[str, float]) -> None:
    FIGURES_DIR.mkdir(parents=True, exist_ok=True)
    ...
    lstm_values = [float(lstm_metrics[key]) for key in metric_keys]
    winner_values = [float(winner_metrics[key]) for key in metric_keys]
    positions = np.arange(len(metric_keys))
    width = 0.35

    fig, ax = plt.subplots(figsize=(10.2, 4.8))
    ax.bar(positions - width / 2, lstm_values, width=width, label="Temporal LSTM", color="#148f77")
    ...
    ax.set_ylim(0.0, 1.0)
    ax.set_ylabel("Score")
    ax.set_title("Temporal LSTM versus final CatBoost winner")
...
def run_experiment() -> dict[str, Any]:
    ...
    summary = {
        "benchmark_name": "lstm_sequence_benchmark",
        ...
        "winner_comparison": {
            ...
            "lstm_minus_winner_roc_auc": float(best_result["winner_test_roc_auc_gap"]),
            "lstm_minus_winner_f1": float(best_result["winner_test_f1_gap"]),
            "lstm_minus_winner_precision": float(best_result["winner_test_precision_gap"]),
            "lstm_minus_winner_recall": float(best_result["winner_test_recall_gap"]),
        },
        "figure_paths": {
